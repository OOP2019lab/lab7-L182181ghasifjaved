...
//	//*****Write your code here
	cout<<"\ntask 3\n";
	bankAccount *b[2];
	float transactions1[3]={3000,5000,60000};
	b[0]=new bankAccount(30,2000,transactions1,3,100000);
...
//	*/
	...
	float transactions2[2]={-5000,5000};
	dyarr[0]=new bankAccount(50,2000,transactions2,2,100000);
...
bankAccount ** readFile(string name,int &size)
{
	...
	{
		fin>>size;
		obj=new bankAccount*[size];
		int i=0;
		while(!fin.eof())
		{
			fin>>id>>balance>>limit>>nooftrans;
			trans=new float[nooftrans];
			for(int j=0;j<nooftrans;j++)
				fin>>trans[j];
			obj[i]=new bankAccount(id,balance,trans,nooftrans,limit);
			i++;
		}
	}
	else
